...
import {Action, Shift, Reduce} from './Action';
  ...
  while (incompleteList.length > 0 || toDoList.length > 0) {
      ...
      for (const item of currentSet) {
        if (item.isShift() && !actions.has(item)) {
          ...
          for (const shItem of currentSet) {
            if (shItem.isShift() && shItem.getMarkedSymbol().equals(symbol)) {
              newItemSet.add(
                new Item(
                  shItem.production,
    ...
    while (toDoList.length > 0) {
      ...
      for (let item of currentSet) {
        if (item.isReduction()) {
          actions.set(item, new Reduce(item.production, item.lookahead));
        }
      }
