...
const symbolArrayEquals = (a: Symbol[], b: Symbol[]) => {
  return (
    a.length === b.length && a.every((entry, i) => entry.equals(b[i]))
  );
};
...
export class Item {
  ...
  public hasReductionConflictWith(item: Item) {
    ...
    return (
      !this.production.leftHandSide.equals(
        item.production.leftHandSide,
      ) &&
  ...
  public equals(item: Item) {
    return (
      this.hasIdenticalCoresWith(item) &&
      this.lookahead.equals(item.lookahead)
    );
  }
  ...
  public toString() {
    const beforeDot = this.production.rightHandSide
      .slice(0, this.dotPosition)
      .map((entry) => entry.toString())
      .join(' ');
    const afterDot = this.production.rightHandSide
    ...
    } ->${beforeDot ? ` ${beforeDot}` : ''} .${afterDot ? ` ${afterDot}` : ''} [ ${this.lookahead.toString()} ]`;
  }
}
